...
public final class RSSAggregator {
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        // Set title, close head tag and open other tags
        fileOut.println("</head>");
        fileOut.println("<body>");
        fileOut.println("<h1> " + title + " </h1>");
        fileOut.println("<ul>");

        // Loop through each of xml's children (the feeds)
        for (int i = 0; i < xml.numberOfChildren(); i++) {

            // Create a simpleWriter initialized with the feed's file name
            SimpleWriter rssOut = new SimpleWriter1L(
                    xml.child(i).attributeValue("file"));

            // Create XMLTree with the rss feed's url
            XMLTree rss = new XMLTree1(xml.child(i).attributeValue("url"));

            // If the feed is a valid RSS
            if (isRSS(rss)) {
                out.println("File #" + (i + 1) + ":" + " Success!");
                // Print a link to the rss feed along with a name
                fileOut.println("<li><a href = "
                        + xml.child(i).attributeValue("file") + " > "
                        + xml.child(i).attributeValue("name") + "</a> </li>");

                // The rest of the loop works the rss link in the feed
                // output the header
                outputHeader(rss.child(0), rssOut);

                // run the "processItem" method for each occurence of the item node
                for (int j = 0; j < rss.child(0).numberOfChildren(); j++) {
                    if (rss.child(0).child(j).label().equals("item")) {
                        processItem(rss.child(0).child(j), rssOut);
                    }
                }
                ...
                // If the feed isn't a valid RSS, do this instead
            } else {
                out.println(
                        "File #" + (i + 1) + ":" + " Error! Invalid RSS File");
            }
            rssOut.close();
        }
        /*
         * if (!isRSS(xml)) { out.println("Invalid RSS 2.0 feed."); //
         * otherwise, The RSS reading process continues } else { // Prompt the
         * user for an HTML file name and set a // simpleWriter to output to the
         * file }
         */
        fileOut.println("</ul>");
        fileOut.println("</body>");
    ...
    public static void main(String[] args) {
        ...
        // Prompt the user for an XML file
        out.print("Enter an XML file containing RSS Feeds: ");
        String userRSS = in.nextLine();

// Prompt the user for an HTML file
        out.print("Enter an HTML file name for output: ");
        String userHTML = in.nextLine();

        // call processFeed
        processFeed(userRSS, userHTML, out);
