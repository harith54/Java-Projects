...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        /*
         * return a substring of str1 starting at 0 and going str1's length
         * minus the overlap with str2 concatenated to make the combination
         */
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Initalize canAdd as true and create a set to
        // store anything that is a substring of str
        boolean canAdd = true;
        Set<String> substringsOfStr = new Set1L<>();

        // Use the for each loop to iterate through the set
        for (String s : strSet) {
            // If the current string is a substring of str, add it to the new set
            if (str.contains(s)) {
                substringsOfStr.add(s);
                // If str is a substring of the current string, canAdd is false
        ...
        /*
         * If canAdd remains true and the set doesn't already contain str, add
         * str to the original set and remove all strings that are substrings of
         * str
         */
        if (canAdd && !strSet.contains(str)) {
            strSet.add(str);
            strSet.remove(substringsOfStr);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // While the end of the file stream has not been reached
        while (!input.atEOS()) {
            // Using the method above, add the current line to the set as a string
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            // If the current character is the separator, print a new line
            if (currentChar == '~') {
                out.println();
